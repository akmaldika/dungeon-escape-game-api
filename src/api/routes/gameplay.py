...
if not logger.handlers:
    ...
    fh = RotatingFileHandler(
        os.path.join("log", "api_actions.log"), maxBytes=5 * 1024 * 1024, backupCount=3
    )
...
def create_router(game_state: ThreadSafeGameState) -> APIRouter:
    ...
    async def start_game(request: StartGameRequest):
        if request.mode not in ["custom", "procedural", "string", "cellular"]:
            raise HTTPException(
                status_code=400,
                detail="Mode must be 'custom', 'procedural', 'cellular', or 'string'",
            )

        if request.mode == "string" and not request.custom_map:
            raise HTTPException(
                status_code=400, detail="custom_map is required when mode is 'string'"
            )
        ...
        if request.mode == "string":
        ...
        elif request.mode in ["procedural", "cellular"]:
            # Include procedural/cellular and FOV parameters
            ...
            action_key = f"restart_{request.mode}|{params}"
        else:
    ...
    async def perform_action(request: PerformActionRequest, http_request: Request):
        ...
        logger.info(
            "perform-action queued: action=%s; client=%s", action_lower, client_host
        )
